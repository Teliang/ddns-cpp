...
void update_pihole(configure &conf, std::string &current_ip)
{
  ...
  std::vector<std::tuple<std::string, std::string>> hosts = get_hosts_config(url, sid);

  auto it = std::find_if(hosts.begin(), hosts.end(),
                         [conf](std::tuple<std::string, std::string> &x)
                         {
                           return std::get<1>(x) == conf.domain;
                         });

  if (it != hosts.end())
  {
    std::cout << "Found ip: " << std::get<0>(*it) << std::endl;
    std::cout << "Found domain: " << std::get<1>(*it) << std::endl;
