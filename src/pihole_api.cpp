...
std::string auth(std::string url, std::string password) {
  ...
  std::string result = "";
  std::string json = std::format(R"foo({{"password":"{}"}})foo", password);
...
get_hosts_config(std::string url, std::string sid) {
  // HTTP
  httplib::Client cli(url);
...
std::string
update_hosts_config(std::string url, std::string sid,
  ...
  if (auto res = cli.Patch("/api/config?restart=true&sid=" + sid, json,
                           "application/json")) {
    if (res->status == httplib::StatusCode::OK_200) {
      std::cout << "http request success, body: " << res->body << std::endl;
      result = res->body;
